...
const RecruiterManageJobsPage = () => {
    ...
    try {
      ...
      if (companyRes.success) {
        ...
        const [jobsRes, appsRes] = await Promise.all([
          getJobsByCompanyId(companyRes.data.id),
          getApplicationsByCompanyApi(companyRes.data.id)
        ]);
        if (jobsRes.success) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 font-sans">
                      ...
                      <tr key={job.id} className="group hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="py-6 px-4">
                          <div className="flex items-center gap-2">
                            <div className="w-8 h-8 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center font-black text-xs">
                              {allApplications.filter(a => a.jobPostId === job.id).length}
                            </div>
                        ...
                        <td className="py-6 px-4">
                          <button
                            ...
                            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border font-black text-[10px] uppercase tracking-wider transition-all cursor-pointer ${job.status === 'Active'
                        ...
                        <td className="py-6 px-4 text-right">
                          <div className="flex items-center justify-end gap-2">
                            ...
                            <button 
                              ...
                              className="w-10 h-10 rounded-xl bg-slate-50 text-slate-400 hover:text-amber-600 hover:bg-amber-50 transition-all flex items-center justify-center cursor-pointer"
                            >
                              <Edit size={18} />
                            </button>
                            <button
      ...
      {company && (
        <PostJobModal
          isOpen={isPostJobModalOpen || !!editingJob}
          onClose={() => {
